...
def predict():
    ...
    discplacement = request.args.get("discplacement", None)
    ...
    data = [cylinders, discplacement, horsepower, weight, acceleration, model_year, origin]

    if None in data:
        return -999
    
    else: 
        pred_df = pd.DataFrame(np.array(data).reshape(1,-1), columns=saved_model.feature_names_in)
